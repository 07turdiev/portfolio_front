...
const OUT_PATH = join(here, '../src/data/projectedMap.js')
...
const SHAPE_TO_SVG = {
  ...
  // Fargona
  'Altyarik': 'far-olt', 'Bagdad': 'far-bag', 'Beshariq': 'far-besh',
  'Buvayda': 'far-buv', 'Dangara': 'far-dan', 'Yazyavan': 'far-yoz',
  'Kuva': 'far-quv', 'Kushtepa': 'far-qush', 'Rishtan': 'far-rish',
  'Sokh': 'far-sox', 'Toshlak': 'far-tosh', 'Uchkuprik': 'far-uchk',
  ...
  // Xorazm
  'Bagat': 'xor-bog', 'Gurlan': 'xor-gur', 'Khazarasp': 'xor-xaz',
  'Khanka': 'xor-xon', 'Khiva': 'xor-xiv', 'Kushkupir': 'xor-qush',
...
writeFileSync(
  OUT_PATH,
  `// Auto-generated by scripts/build-geo-map.mjs — do not edit by hand.\n` +
    `export const COUNTRY_VIEWBOX = ${JSON.stringify(output.COUNTRY_VIEWBOX)}\n` +
    `export const regions = ${JSON.stringify(output.regions)}\n` +
    `export const regionViewBoxes = ${JSON.stringify(output.regionViewBoxes)}\n` +
    `export const districtsByRegion = ${JSON.stringify(output.districtsByRegion)}\n` +
    `export const districtNameMap = ${JSON.stringify(output.districtNameMap)}\n`
)
...
console.log(
  `Wrote ${OUT_PATH} — regions=${regions.length}, districts=${totalDistricts}`
)
